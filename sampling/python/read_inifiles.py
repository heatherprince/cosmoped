...
def read_compression_vectors_and_compressed_data(path, with_low_ell):
    if with_low_ell:
        filename_extra='with_low_ell'
    else:
        filename_extra='high_ell_only'

    f=open(path+'compressed_planck_data_'+filename_extra+'.dat', 'r+')
    ...
    for p in param_names:
        compression_vector_dict[p]=np.loadtxt(path+p+'_compression_vector_'+filename_extra+'.dat')
